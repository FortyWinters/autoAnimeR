...
pub async fn do_spider_task(
    ...
    if !subscribed_anime_vec.is_empty()  {
        let task_res_vec = join_all(subscribed_anime_vec
            .into_iter()
            .map(|a|{
                get_st_anime(a, &mikan)
            })).await;
    ...
    for st_a in &st_anime_vec {
        st_subgroup_map.entry(st_a.subgroup_id).or_insert(st_a.subgroup_name.to_string());
    }
    let anime_subgroup_vec: Vec<models::anime_subgroup::AnimeSubgroupJson> = st_subgroup_map
        ...
        .collect();
    dao::anime_subgroup::add_vec(db_connection, anime_subgroup_vec).await.unwrap();

    if !st_anime_vec.is_empty() {
        let task_res_vec = join_all(st_anime_vec
            .into_iter()
            .map(|st_a|{
                get_st_seed(st_a.mikan_id, st_a.subgroup_id, st_a.anime_type, &mikan)
            })).await;
...
pub async fn get_st_anime(
    ...
    match mikan.get_subgroup(a.mikan_id).await {
        Ok(s_vec) => {
            Ok(convert_to_spidertaskanime(a, &s_vec))
        },
        Err(_) => Err(())
    }
}
...
pub fn convert_spiderseed_to_animeseed(spider_vec: &Vec<spider::Seed>) -> Vec<models::anime_seed::AnimeSeedJson> {
    spider_vec.into_iter().map(|s| models::anime_seed::AnimeSeedJson {     
        mikan_id    : s.mikan_id,
        subgroup_id : s.subgroup_id,
        episode     : s.episode,
