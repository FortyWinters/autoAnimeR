...
impl DoAnimeTask {
    ...
    pub async fn create_anime_task_bulk(&mut self) -> Result<(), Error>{
        ...
        // 过滤并下载
        let anime_task_set = dao::anime_task::get_exist_anime_task_set(&mut self.db_connection).await.unwrap();
        self.filter_and_download(anime_seed_vec, anime_task_set).await.unwrap();
    ...
    pub async fn create_anime_task_single(
        ...
        let anime_task_set = dao::anime_task::get_exist_anime_task_set_by_mikanid(
            &mut self.db_connection, 
            mikan_id)
            .await
            .unwrap();
    ...
    pub async fn filter_and_download (
        ...
        anime_task_set: HashSet<(i32, i32)>,
    ) -> Result<(), Error> {
...
mod test {
    use super::*;
    use crate::models::anime_seed::AnimeSeedJson;
    use crate::models::anime_subgroup::AnimeSubgroupJson;
    ...
    pub async fn test_create_anime_task() {
        ...
        // reset 
        dao::anime_seed::delete_anime_seed_by_mikan_id(&mut do_anime_task.db_connection, 3143).await.unwrap();
        dao::anime_task::delete_anime_task_by_mikan_id(&mut do_anime_task.db_connection, 3143).await.unwrap();

        dao::anime_seed::add(&mut do_anime_task.db_connection, test_anime_seed_json).await.unwrap();
        dao::anime_subgroup::add(&mut do_anime_task.db_connection, test_anime_subgroup).await.unwrap();

        // let _r = do_anime_task.create_anime_task_bulk().await.unwrap();
        let _r = do_anime_task.create_anime_task_single(3143, 3).await.unwrap();
