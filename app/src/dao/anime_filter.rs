...
pub async fn add_global_subgroup_filter(
        ...
        .first::<AnimeFilter>(db_connection)
    {
        ...
        Err(_) => {
            ...
            let result = anime_filter.order(id.desc()).first(db_connection).unwrap();
            Ok(result)
        }
    }
}
...
pub async fn get_local_episode_filter_by_mikan_id(
    ...
    match anime_filter
        ...
        .first::<AnimeFilter>(db_connection)
    {
        Ok(result) => Ok(result.filter_val),
        Err(_) => Ok(0),
    }
}
...
mod test {
    ...
    async fn test() {
        ...
        add_local_subgroup_filter_by_mikan_id(114, 514, db_connection)
            .await
            .unwrap();
        add_local_subgroup_filter_by_mikan_id(114, 114514, db_connection)
            .await
            .unwrap();
        add_local_subgroup_filter_by_mikan_id(114, 1919810, db_connection)
            .await
            .unwrap();

        delete_local_subgroup_filter_by_mikan_id(114, 514, db_connection)
            .await
            .unwrap();
    }
}
