...
pub async fn filter_anime_bulk_with_anime_filter(
    ...
    let mut new_anime_seed_vec: Vec<AnimeSeed> = Vec::new();
    let global_subgroup_filter = dao::anime_filter::get_global_subgroup_filter_set(db_connection).await;

    for anime_seed in anime_seed_vec {
        let local_subgroup_filter = dao::anime_filter::get_local_subgroup_filter_set_by_mikan_id(anime_seed.mikan_id, db_connection).await.unwrap();
        let local_episode_filter = dao::anime_filter::get_local_episode_filter_by_mikan_id(&anime_seed.mikan_id, db_connection).await.unwrap();
